...
Если вам нужно синхронизировать конкретный пост, запустите приложение с параметром `-p <id_поста>`. Можно указать автора через `-a <имя_автора>` или после программа спросит автора сама:

```shell
./boosty_downloader.exe -p abcdefgh-ijkl-mnop-qrst-uvwxyz123456 -a autorsname
```

Также можно передать ссылку через `-l <полная_ссылка>`
```shell
./boosty_downloader.exe -l "https://boosty.to/autorsname/posts/abcdefgh-ijkl-mnop-qrst-uvwxyz123456"
```

Или же несколько ссылок через `-f <ссылка_на_файл>`
```shell
./boosty_downloader.exe -f "C:\Users\User\Desktop\links.txt"
```
